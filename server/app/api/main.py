...
from app.core.utils.load_model import get_dense_model, get_reranker
...
app = FastAPI(
    title="HUST Q&A API", description="Retrieving documents and generating answers"
)
...
async def signup(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    return UserResponse(user=User.from_orm(new_user), token=access_token)
...
async def retrieve(query: QueryModel, current_user: DBUser = Depends(get_current_user)):
    ...
    return DocumentResponse(
        documents=reranked if reranked else sparse_docs[: bm25_config.top_k]
    )


@app.post("/api/gemini", response_model=GeminiResponse)
async def ask_gemini(
    query: QueryModel, current_user: DBUser = Depends(get_current_user)
):
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def ask_with_context(
    ...
    dense_docs = dense_model(query.query, sparse_docs)

    print(dense_docs)

    reranked = None
    try:
        scores = reranker(query.query, dense_docs)
        pairs = list(zip(dense_docs, scores))
        pairs.sort(key=lambda x: x[1], reverse=True)
        reranked = [doc for doc, score in pairs]
    except Exception as e:
        print(f"Error during reranking: {e}")
        reranked = None

    relevant_docs = reranked if reranked else dense_docs
